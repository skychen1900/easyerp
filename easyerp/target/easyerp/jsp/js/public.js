...
function openDeleteDialog(url,confirmString){
	...
	if(confirm(c)){
		...
		var win = window.open(url,"删除确认","width=" + iWidth + ", height=" + iHeight + ",top=" + iTop + ",left=" + iLeft + ",toolbar=no, menubar=no, scrollbars=no, resizable=no,location=no, status=no,alwaysRaised=yes,depended=yes");
	}
	return false;
}
